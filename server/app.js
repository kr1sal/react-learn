...
function searchEmojis(query, data) {
  ...
  return data.filter((emoji) => {
    ...
    const matchesKeywords = emoji.keywords
      .toLowerCase()
      .includes((keyword) => keyword.toLowerCase().includes(lowerQuery));
...
app.listen(PORT, () => {
  ...
  console.log(`  - Все эмодзи: http://localhost:${PORT}/api/emojis`);
  console.log(`  - Поиск: http://localhost:${PORT}/api/emojis?q=smile`);
  console.log(
    `  - По ключевому слову: http://localhost:${PORT}/api/emojis?q=fire`
  );
});
